...
def build_hazy_dataset(
    ...
    name = dataset.lower()
    if name == "mnist":
        return HazyMNIST(root=data_dir, train=train, img_size=img_size, fixed_fog=fixed_fog)
    if name in ("cifar", "cifar10"):
        if img_size != 32:
            raise ValueError("CIFAR-10 dehazing uses fixed image size 32.")
        return HazyCIFAR10(root=data_dir, train=train, fixed_fog=fixed_fog)
...
def train(
    ...
    train_set = build_hazy_dataset(name, data_dir, train=True, img_size=img_size)
    vis_set = build_hazy_dataset(name, data_dir, train=False, img_size=img_size, fixed_fog=True)
    ...
    def save_random_triple_grid(path: Path, n: int = 16) -> Dict[str, float]:
        ...
        return save_triple_grid(ema.shadow, vis_clean, vis_hazy, path)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x_clean, x_hazy, _label) in enumerate(train_loader):
            ...
            if x_hat.shape != x_clean.shape:
                raise RuntimeError(f"Model output shape {tuple(x_hat.shape)} != clean {tuple(x_clean.shape)}")
...
def save_triple_grid(
    ...
    metrics = paired_dehaze_metrics(x_hat, x_clean)
    ...
    save_metrics(path.with_suffix(".txt"), metrics)
    if was_training:
        model.train()
    return metrics


def paired_dehaze_metrics(x_hat: torch.Tensor, x_clean: torch.Tensor) -> Dict[str, float]:
    """Simple paired synthetic metrics on [-1, 1] tensors."""
    mse = F.mse_loss(x_hat, x_clean).item()
    psnr = 20.0 * math.log10(2.0) - 10.0 * math.log10(max(mse, 1e-12))
    return {"mse": mse, "psnr": psnr}


def format_metrics(metrics: Dict[str, float]) -> str:
    return f"MSE {metrics['mse']:.6f} | PSNR {metrics['psnr']:.2f} dB"
